//DIJKSTRA'S ALGORITHM USING DYNAMIC PROGRAMMING
...
char nodes_name[MAX] = {'A','N','N','E','B','D','C'};
...
void main()
{
	...
	for(i=1;i<n;i++)
	{
	printf("\n\n The distance of %c from 'A' is %d unit.",nodes_name[vertex_array[i]],dist[i]);
	}
...
Dijkstras_algorithm(int n,int s)
{
	...
	for(i=0;i<n;i++)
	{
		...
		for(j=0;j<n;j++)
		{
			if(cost[arr[i].vertex_name][j] != 0)
			{
				wt = cost[arr[i].vertex_name][j];
				
				Relax(arr[i].vertex_name,j,n,wt);
